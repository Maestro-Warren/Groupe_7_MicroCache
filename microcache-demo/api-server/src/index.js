...
cache.connect()
  .then(() => console.log(`[CACHE] Connecté à MicroCache sur ${CACHE_HOST}:${CACHE_PORT}`))
  .catch(err => console.warn(`[CACHE] Non disponible (mode dégradé): ${err.message}`));
...
function broadcast(data) {
  ...
  wss.clients.forEach(client => {
    if (client.readyState === 1) { // OPEN
      client.send(msg, err => { if (err) console.error('[WS] Send error:', err.message); });
    }
  });
}
